...
// namespace to limit, a decimal string.
void SetMaxUserNamespacesOrDie(const char* limit) {
  WriteFileOrDie(kMaxUserNamespaces, limit, strlen(limit));
}

// Installs uid/gid maps for the calling process's own user namespace. A
void MapSelfUserNamespaceOrDie(const char* uid_map, const char* gid_map) {
  WriteFileOrDie("/proc/self/uid_map", uid_map, strlen(uid_map));
  WriteFileOrDie("/proc/self/setgroups", "deny", 4);
  WriteFileOrDie("/proc/self/gid_map", gid_map, strlen(gid_map));
}
...
TEST(ProcSysUserMaxUserNamespaces, LimitBlocksNestedUserNamespace) {
  ...
  // Mapping the namespace from the parent (rather than from /proc/self) keeps
  // teardown clean.
...
TEST(ProcSysUserMaxUserNamespaces, RecursiveLimitBlocksGrandchild) {
  ...
  EXPECT_THAT(InForkedProcess([&uid_map, &gid_map] {
                ...
                TEST_PCHECK(unshare(CLONE_NEWUSER) == 0);
                MapSelfUserNamespaceOrDie(uid_map.c_str(), gid_map.c_str());
                SetMaxUserNamespacesOrDie("1");

                // Child namespace B succeeds and consumes A's budget. The
                // mapped uid is now 0, so map it to itself.
                TEST_PCHECK(unshare(CLONE_NEWUSER) == 0);
                MapSelfUserNamespaceOrDie("0 0 1", "0 0 1");

                // The grandchild is still charged to A, whose budget is gone,
                // so the unshare must fail with ENOSPC rather than succeeding.
                TEST_CHECK_ERRNO(unshare(CLONE_NEWUSER), ENOSPC);
              }),
              IsPosixErrorOkAndHolds(0));
}
...
TEST(ProcSysUserMaxUserNamespaces, RefusedNestingReleasesPartialCharge) {
  ...
  EXPECT_THAT(InForkedProcess([&uid_map, &gid_map] {
                ...
                if (child == 0) {
                  ...
                  TEST_PCHECK(unshare(CLONE_NEWUSER) == 0);
                  MapSelfUserNamespaceOrDie("0 0 1", "0 0 1");
                  // B has room of its own, so each attempt below is charged to
                  // B before A is reached and refuses it.
                  SetMaxUserNamespacesOrDie("2");
                  TEST_CHECK_ERRNO(unshare(CLONE_NEWUSER), ENOSPC);
                  TEST_CHECK_ERRNO(unshare(CLONE_NEWUSER), ENOSPC);

                  TEST_PCHECK(close(ready[1]) == 0);  // ask A to make room
                  char x;
                  RetryEINTR(read)(go[0], &x, 1);

                  // Both refused attempts must have been refunded to B;
                  // otherwise B sits at its own limit of two and this fails
                  // with ENOSPC.
                  TEST_PCHECK(unshare(CLONE_NEWUSER) == 0);
                  _exit(0);
                }
